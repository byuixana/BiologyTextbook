...
const DEFAULT_DECK_LINK = './histology_questions.json';
...
function AppContent() {
  ...
  useEffect(() => {
    ...
    .then(data => {
      ...
      const cardArray = Object.entries(data).map(([id, card]) => ({
        ...card,
        id: Number(id)
      }));
  ...
  return (
    <>
      <div className='flex 
        ...
        p-4
        shadow-md'> 
        <h1 className='m-0 box-border w-full px-3 text-center text-2xl font-bold sm:text-3xl'>Bio 264 Exam 1 Flashcards</h1>
      </div>
