...
const labs = [
  ...
  'Exam3/ArmMusclesLab',
  'Exam3/LegMusclesLab',
  'ExamFinal/NervousSystemLab',
  'ExamFinal/SpecialSensesLab'
];
...
for (const lab of labs) {
  ...
  const buildDir = path.join(labRoot, 'build');
  const publicLabDataPath = path.join(publicDir, 'lab_data.json');
  if (!fs.existsSync(publicLabDataPath)) {
    console.log(`${lab}: skipped (no public/lab_data.json)`);
    continue;
  }
  ...
  for (const section of data.sections || []) {
    walk(section.data, (item) => {
      ...
      if (!/video/i.test(item.load)) return; // only convert video-designated HTML pages
